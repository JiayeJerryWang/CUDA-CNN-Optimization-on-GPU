...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    */
    ...
    for (int c = 0; c < Channel; c++) { 
        for (int p = 0; p < K; p++) {
            for (int q = 0; q < K; q++) {
                acc = __hadd2_rn(acc, __hmul2_rn(
                    __halves2half2(__float2half(in_4d(b, c, h + p, w + q)), __float2half(in_4d(b + Batch / 2, c, h + p, w + q))), 
                    __halves2half2(__float2half(mask_4d(m, c, p, q)), __float2half(mask_4d(m, c, p, q))))
                );
            }
        }
        __syncthreads();
    ...
    if (b < Batch && m < Map_out && h < Height_out && w < Width_out) {
        ...
        out_4d(b + Batch / 2, m, h, w) = __high2float(acc);
    }
    #undef out_4d
    #undef in_4d
    #undef mask_4d
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    dim3 gridDim(Map_out, Y, Batch / 2);
