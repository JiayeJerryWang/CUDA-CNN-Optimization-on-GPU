...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    */
    ...
    #define tile(i2, i1, i0) tile[(i2) * tile_width * tile_width + (i1) * tile_width + i0]
    ...
    for (int c = 0; c < Channel; c++) {
        if((h < Height) && (w < Width)) {
            tile[threadIdx.y * tile_width + threadIdx.x] = in_4d(b, c, h, w);
        } else {
            tile[threadIdx.y * tile_width + threadIdx.x] = 0.0f;
        }
        __syncthreads();
        if (threadIdx.y < TILE_WIDTH && threadIdx.x < TILE_WIDTH) {
            for (int p = 0; p < K; p++) {
                for (int q = 0; q < K; q++) {
                    acc += tile[(threadIdx.y + p) * tile_width + threadIdx.x + q] * mask_4d(m, c, p, q);
                }
            }
        }
    }
